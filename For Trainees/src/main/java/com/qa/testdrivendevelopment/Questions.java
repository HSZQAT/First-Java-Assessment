...
public class Questions {
	...
	public String[] sortAlphabetically(String[] wordSequence) {
		String[] alphabetical = new String[wordSequence.length];
		for (int i = 0; i < wordSequence.length; i++) {
			for (int j = i + 1; j < wordSequence.length; j++) {

				if (wordSequence[i].compareTo(wordSequence[j]) < 0) {

					alphabetical[i] = wordSequence[i];
					alphabetical[j] = wordSequence[j];

				} else if (wordSequence[i].compareTo(wordSequence[j]) > 0) {

					alphabetical[i] = wordSequence[j];
					alphabetical[j] = wordSequence[i];
	...
	public int aPlus(int num) {
		...
		result = num + (num * 11) + ((num * 11) + (num * 100)) + ((num * 1000) + (num * 100) + (num * 11));
		return result;
	}
	...
	public String oddNum(String allNums) {
		...
		String[] numStringList = allNums.split(",");
		...
		for (int i = 0; i < oddList.length; i++) {
			...
			String oddNum = String.parseString(oddList[i]);
			result = oddNum;
	...
	public int fibonacci(int num) {
		...
		int[] fibonacciAnswer = new int[num + 2];
		fibonacciAnswer[0] = 0;
		fibonacciAnswer[1] = 1;

		for (int i = 2; i < num + 2; i++) {

			fibonacciAnswer[i] = fibonacciAnswer[i - 1] + fibonacciAnswer[i - 2];

		}

		return fibonacciAnswer[num];
	}
	...
	public String oddLetters(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			if (i % 2 != 0) {
				char seperateChar = input.charAt(i);
				result += seperateChar;
